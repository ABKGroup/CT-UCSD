...
# 6 tmux sessions for data collection.
for i in $(seq 103 108); do
...
# 8 tmux sessions for data collection.
for i in $(seq 145 152); do
    tmux new-session -d -s collect_job_$(printf "%02d" $((i)))
    tmux send-keys -t collect_job_$(printf "%02d" $((i))) "export CUDA_VISIBLE_DEVICES=-1" Enter
...
tmux attach-session -t collect_job_150
